...
def test_openai_summarizer_minimax_provider_defaults():
    ...
    mock_client.post.assert_called_once()
    assert (
        mock_client.post.call_args[0][0] == "https://api.minimax.io/v1/chat/completions"
    )
...
def test_openai_summarizer_openrouter_provider_defaults():
    ...
    assert (
        mock_client.post.call_args[1]["json"]["model"]
    ...
    with (
        ...
        patch(
            ...
            side_effect=lambda k, d=None, **kwargs: (
                True if k == "allow_remote_summarizer" else d
            ),
        ),
...
def test_get_model_name_returns_value_when_set():
    ...
    with patch(
        "jcodemunch_mcp.summarizer.batch_summarize._config.get",
        side_effect=lambda k, d=None, **kwargs: (
            "my-custom-model" if k == "summarizer_model" else d
        ),
    ):
...
def test_get_model_name_strips_whitespace():
    ...
    with patch(
        "jcodemunch_mcp.summarizer.batch_summarize._config.get",
        side_effect=lambda k, d=None, **kwargs: (
            "  claude-haiku  " if k == "summarizer_model" else d
        ),
    ):
...
def test_create_summarizer_disabled_when_string_false(falsy_val):
    ...
    with patch(
        "jcodemunch_mcp.summarizer.batch_summarize._config.get",
        side_effect=lambda k, d=None, **kwargs: (
            falsy_val if k == "use_ai_summaries" else d
        ),
    ):
        assert _create_summarizer() is None
...
def test_create_summarizer_auto_mode_no_providers(monkeypatch):
    ...
    with patch(
        "jcodemunch_mcp.summarizer.batch_summarize._config.get",
        side_effect=lambda k, d=None, **kwargs: (
            "auto" if k == "use_ai_summaries" else d
        ),
    ):
        assert _create_summarizer() is None
...
def test_create_summarizer_auto_mode_detects_provider(monkeypatch):
    ...
    try:
        ...
        with patch(
            ...
            side_effect=lambda k, d=None, **kwargs: (
                "auto"
                if k == "use_ai_summaries"
                else ""
                if k == "summarizer_model"
                else True
                if k == "allow_remote_summarizer"
                else d
            ),
        ):
...
def test_create_summarizer_explicit_true_no_provider_warns_and_autodetects(
    monkeypatch, caplog
):
    ...
    with (
        patch(
            ...
            side_effect=lambda k, d=None, **kwargs: (
                ...
                if k in ("summarizer_provider", "summarizer_model")
                else d
            ),
        ),
        caplog.at_level(
            logging.WARNING, logger="jcodemunch_mcp.summarizer.batch_summarize"
        ),
    ):
    ...
    with patch.dict(sys.modules, {"anthropic": mock_anthropic_module}):
        with patch.dict(
            "os.environ",
            {
                ...
                "ANTHROPIC_MODEL": "claude-haiku-fallback",
            },
            clear=True,
        ):
            with patch(
                "jcodemunch_mcp.summarizer.batch_summarize._config.get",
                side_effect=lambda k, d=None, **kwargs: (
                    "claude-override-model" if k == "summarizer_model" else d
                ),
            ):
                from jcodemunch_mcp.summarizer.batch_summarize import BatchSummarizer
    ...
    with patch.dict(
        ...
        with patch.dict(
            ...
            with patch(
                "jcodemunch_mcp.summarizer.batch_summarize._config.get",
                side_effect=lambda k, d=None, **kwargs: (
                    "gemini-override-model" if k == "summarizer_model" else d
                ),
            ):
...
def test_circuit_breaker_trips_after_consecutive_failures():
    ...
    with patch(
        ...
        side_effect=lambda k, d=None, **kwargs: (
            3
            if k == "summarizer_max_failures"
            else 1
            if k == "summarizer_concurrency"
            else d
        ),
    ):
...
class TestProjectAwareSummarizer:
    ...
    def test_create_summarizer_without_repo_keeps_global_only(
