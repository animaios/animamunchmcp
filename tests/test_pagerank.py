...
class TestComputePagerank:
    ...
    def test_scores_sum_to_one(self):
        ...
        imports = self._make_imports(
            [("a.py", "b.py"), ("c.py", "b.py"), ("d.py", "b.py")]
        )
    ...
    def test_self_import_ignored(self):
        ...
        imports = {
            "a.py": [
                {"specifier": "a.py", "names": []},
                {"specifier": "b.py", "names": []},
            ]
        }
        in_deg, out_deg = compute_in_out_degrees(imports, files)
...
# ---------------------------------------------------------------------------
# Integration tests: get_repo_map(group_by="flat")
# ---------------------------------------------------------------------------


class TestGetRepoMapFlat:
    """Integration tests for the get_repo_map tool with group_by='flat'."""
    ...
    def indexed_repo(self, tmp_path):
        ...
        (src / "standalone.py").write_text("def unused(): pass\n")
    ...
    def test_most_imported_symbol_ranks_first(self, indexed_repo):
        ...
        result = get_repo_map(
            repo_id, group_by="flat", top_n=10, storage_path=str(store)
        )
        assert "error" not in result
    ...
    def test_response_shape(self, indexed_repo):
        ...
        repo_id, store = indexed_repo
        result = get_repo_map(repo_id, group_by="flat", storage_path=str(store))
    ...
    def test_degree_algorithm(self, indexed_repo):
        ...
        repo_id, store = indexed_repo
        result = get_repo_map(
            repo_id, group_by="flat", algorithm="degree", storage_path=str(store)
        )
        assert "error" not in result
    ...
    def test_degree_same_top_as_pagerank(self, indexed_repo):
        ...
        repo_id, store = indexed_repo
        pr_result = get_repo_map(
            repo_id, group_by="flat", algorithm="pagerank", storage_path=str(store)
        )
        deg_result = get_repo_map(
            repo_id, group_by="flat", algorithm="degree", storage_path=str(store)
        )
    ...
    def test_invalid_algorithm(self, indexed_repo):
        ...
        repo_id, store = indexed_repo
        result = get_repo_map(
            repo_id, group_by="flat", algorithm="louvain", storage_path=str(store)
        )
    ...
    def test_top_n_respected(self, indexed_repo):
        ...
        repo_id, store = indexed_repo
        result = get_repo_map(
            repo_id, group_by="flat", top_n=1, storage_path=str(store)
        )
        assert "error" not in result
    ...
    def test_missing_repo(self, tmp_path):
        ...
        result = get_repo_map(
            "nonexistent_repo_xyz", group_by="flat", storage_path=str(tmp_path / ".idx")
        )
        assert "error" in result
...
class TestSearchSymbolsSortBy:
    ...
    def test_relevance_is_default(self, indexed_repo):
        ...
        result = search_symbols(
            repo_id, query="Engine", sort_by="relevance", storage_path=str(store)
        )
        assert "error" not in result
    ...
    def test_centrality_sort(self, indexed_repo):
        ...
        repo_id, store = indexed_repo
        result = search_symbols(
            repo_id, query="Engine", sort_by="centrality", storage_path=str(store)
        )
        assert "error" not in result
        assert result["result_count"] > 0
    ...
    def test_combined_sort(self, indexed_repo):
        ...
        repo_id, store = indexed_repo
        result = search_symbols(
            repo_id, query="helper", sort_by="combined", storage_path=str(store)
        )
        assert "error" not in result
        assert result["result_count"] > 0
    ...
    def test_invalid_sort_by(self, indexed_repo):
        ...
        repo_id, store = indexed_repo
        result = search_symbols(
            repo_id, query="Engine", sort_by="magic", storage_path=str(store)
        )
        assert "error" in result
