"""Tests for v1.14: get_call_hierarchy (including include_impact mode), and related upgrades."""
...
class TestContentCache:
    def _load(self, repo, store_path):
        ...
        from jcodemunch_mcp.tools.get_blast_radius import _find_symbol
    ...
    def test_cache_does_not_change_finder_output(self, tmp_path):
        from jcodemunch_mcp.tools._call_graph import _ContentCache, find_direct_callers

        index, store, owner, name, sym, reverse_adj, sbf = self._load(
            *_build_fanin_repo(tmp_path)
        )
        ...
        cached = find_direct_callers(
            ...
            reverse_adj,
            sbf,
            _ContentCache(store, owner, name),
        )
    ...
    def test_bfs_finds_full_tree_without_reading_a_file_twice(self, tmp_path):
        ...
        results, depth = bfs_callers(
            index, counting, owner, name, sym, reverse_adj, sbf, max_depth=3
        )
...
class TestGetCallHierarchy:
    ...
    def test_callers_found_for_helper(self, tmp_path):
        ...
        assert "process" in caller_names, (
            f"Expected 'process' in callers, got: {caller_names}"
        )
    ...
    def test_callers_direction_returns_no_callees(self, tmp_path):
        repo, store = _build_repo(tmp_path)
        result = get_call_hierarchy(
            repo=repo, symbol_id="helper", direction="callers", storage_path=store
        )
    ...
    def test_callees_direction_returns_no_callers(self, tmp_path):
        repo, store = _build_repo(tmp_path)
        result = get_call_hierarchy(
            repo=repo, symbol_id="process", direction="callees", storage_path=store
        )
    ...
    def test_unknown_symbol_returns_error(self, tmp_path):
        repo, store = _build_repo(tmp_path)
        result = get_call_hierarchy(
            repo=repo, symbol_id="nonexistent_xyz", storage_path=store
        )
    ...
    def test_leaf_symbol_has_no_callers(self, tmp_path):
        ...
        repo, store = _build_repo(tmp_path)
        result = get_call_hierarchy(
            repo=repo, symbol_id="run", direction="callers", depth=3, storage_path=store
        )
        assert "error" not in result
        assert result["callers"] == []
...
# ---------------------------------------------------------------------------
# include_impact mode integration tests (formerly get_impact_preview)
# ---------------------------------------------------------------------------


class TestCallHierarchyIncludeImpact:
    def test_returns_expected_shape(self, tmp_path):
        repo, store = _build_repo(tmp_path)
        result = get_call_hierarchy(
            repo=repo,
        ...
        assert "affected_symbol_count" in impact
        assert "affected_symbols" in impact
        assert "affected_by_file" in impact
        assert "call_chains" in impact
        assert "_meta" in impact
    ...
    def test_affected_symbols_includes_callers(self, tmp_path):
        ...
        names = [s["name"] for s in result["impact"]["affected_symbols"]]
        assert "process" in names
    ...
    def test_affected_files_count(self, tmp_path):
        ...
        assert result["impact"]["affected_files"] >= 1
    ...
    def test_call_chains_present(self, tmp_path):
        ...
        impact = result["impact"]
        assert isinstance(impact["call_chains"], list)
        if impact["call_chains"]:
            chain_entry = impact["call_chains"][0]
    ...
    def test_meta_source_is_ast_call_references(self, tmp_path):
        ...
        assert result["impact"]["_meta"]["source"] == "ast_call_references"
    ...
    def test_call_chain_ends_at_caller(self, tmp_path):
        ...
        for entry in result["impact"]["call_chains"]:
            # chain[-1] == symbol_id
    ...
    def test_no_duplicate_affected_symbols(self, tmp_path):
        ...
        ids = [s["id"] for s in result["impact"]["affected_symbols"]]
    ...
    def test_leaf_symbol_has_no_affected(self, tmp_path):
        """run() has no callers — impact should be empty."""
        repo, store = _build_repo(tmp_path)
        result = get_call_hierarchy(
            repo=repo,
            symbol_id="run",
        ...
        assert "error" not in result
        assert result["impact"]["affected_symbol_count"] == 0
        assert result["impact"]["affected_files"] == 0
    ...
    def test_affected_by_file_keys_match_affected_symbols(self, tmp_path):
        ...
        files_in_list = {s["file"] for s in result["impact"]["affected_symbols"]}
        files_in_dict = set(result["impact"]["affected_by_file"].keys())
...
class TestBlastRadiusCallDepth:
    def test_call_depth_zero_no_callers_field(self, tmp_path):
        ...
        result = get_blast_radius(
            repo=repo, symbol="helper", depth=1, call_depth=0, storage_path=store
        )
    ...
    def test_call_depth_one_adds_callers_field(self, tmp_path):
        repo, store = _build_repo(tmp_path)
        result = get_blast_radius(
            repo=repo, symbol="helper", depth=1, call_depth=1, storage_path=store
        )
        assert "callers" in result
        assert "caller_count" in result
    ...
    def test_call_depth_clamped_to_3(self, tmp_path):
        ...
        result = get_blast_radius(
            repo=repo, symbol="helper", depth=1, call_depth=99, storage_path=store
        )
        assert "error" not in result
        assert "callers" in result
...
class TestFindReferencesCallChain:
    ...
    def test_include_call_chain_adds_calling_symbols(self, tmp_path):
        ...
        result = find_references(
            repo=repo, identifier="helper", storage_path=store, include_call_chain=True
        )
        assert "error" not in result
...
class TestSameFileCallers:
    ...
    def test_same_file_caller_found(self, tmp_path):
        ...
        caller_names = [c["name"] for c in result["callers"]]
        assert "process" in caller_names, (
            f"Expected 'process' in same-file callers, got: {caller_names}"
        )
