...
def generate_template() -> str:
    ...
    # Removed (v1.80.8): analyze_perf, audit_agent_config, check_embedding_drift,
    #   diff_health_radar, get_cross_repo_map, get_delivery_metrics, get_file_risk,
    #   get_group_contracts, get_redaction_log, get_symbol_diff, get_watch_status,
    #   invalidate_cache, suggest_corrections, tune_weights
    # Unified reading tools (v1.81+): index_content, list_content, get_outline, get_file,
    #   search_units, get_unit, get_unit_context replace index_repo, index_folder,
    #   get_file_tree, get_file_outline, get_file_content, search_symbols, get_symbol_source, get_context_bundle
    all_tools = sorted(
        [
...
{{
  ...
  //       fails.
  //
  //   To switch an existing index: delete the index first, then
  ...
  //   ~/.code-index/telemetry.db. The in-memory ring is always
  //   tracked; this flag only controls durable persistence.
