    ...
    for qh, rows in by_qh.items():
        if len(rows) >= REQUERY_LIFETIME:
            out.append(
                _cluster(
                    "requery_churn",
                    _sev(len(rows), REQUERY_HIGH, REQUERY_LIFETIME),
                    rows,
                    query_hash=qh,
                    repeats=len(rows),
                )
...
def _detect_low_confidence(by_qh: "dict[str, list[tuple]]") -> list[dict]:
    ...
    for qh, rows in by_qh.items():
        hits = [
            r
            for r in rows
            if r[_CONF] is not None
            and r[_CONF] < LOW_CONF
            and _decode_ids(r[_RETURNED])
        ]
        if len(hits) >= LOW_CONF_RECUR:
            ...
            out.append(
                _cluster(
                    "low_confidence",
                    _sev(len(hits), LOW_CONF_RECUR * 3, LOW_CONF_RECUR),
                    hits,
                    query_hash=qh,
...
def _detect_ambiguous_top(by_qh: "dict[str, list[tuple]]") -> list[dict]:
    ...
    for qh, rows in by_qh.items():
        hits = [
            r
            for r in rows
            if r[_TOP1] is not None
            and r[_TOP2] is not None
        ...
        if len(hits) >= AMBIGUOUS_RECUR:
            out.append(
                _cluster(
                    "ambiguous_top",
                    _sev(len(hits), AMBIGUOUS_RECUR * 3, AMBIGUOUS_RECUR),
                    hits,
                    query_hash=qh,
                    min_gap=round(min(r[_TOP1] - r[_TOP2] for r in hits), 4),
                )
            )
    out.sort(key=lambda c: -c["event_count"])
    return out[:MAX_CLUSTERS_PER_SIGNAL]
...
def _detect_vocabulary_gap(by_qh: "dict[str, list[tuple]]") -> list[dict]:
    ...
    for qh, rows in by_qh.items():
        hits = [
            ...
            if not r[_IDHIT]
            and r[_SEM]
            and r[_CONF] is not None
            and r[_CONF] >= VOCAB_CONF_FLOOR
        ]
        if len(hits) >= VOCAB_RECUR:
            out.append(
                _cluster(
                    "vocabulary_gap",
                    _sev(len(hits), VOCAB_RECUR * 3, VOCAB_RECUR),
                    hits,
                    query_hash=qh,
                    identity_misses=len(hits),
                )
            )
    out.sort(key=lambda c: -c["event_count"])
    return out[:MAX_CLUSTERS_PER_SIGNAL]
