...
ss.add_argument("--lang")
ss.add_argument("-n", type=int, default=10)

gs = sub.add_parser("get")
gs.add_argument("repo")
gs.add_argument("symbol_id")
...
fc.add_argument("repo")
fc.add_argument("file_path")
fc.add_argument("--start", type=int)
fc.add_argument("--end", type=int)
...
if args.cmd == "list":
    ...
    out(
        index_folder(t)
        if "/" not in t or t.startswith("/") or (len(t) > 1 and t[1] == ":")
        else index_repo(t)
    )
elif args.cmd == "outline":
    out(
        get_file_outline(args.repo, args.file)
        if args.file
        else get_repo_outline(args.repo)
    )
    ...
    out(get_symbol(args.repo, args.symbol_id))
elif args.cmd == "text":
    out(search_text(args.repo, args.query, context_lines=args.context_lines))
elif args.cmd == "file":
    out(
        get_file_content(
            args.repo, args.file_path, start_line=args.start, end_line=args.end
        )
    )
